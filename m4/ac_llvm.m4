...
AC_ARG_WITH([llvm],
    ...
    if test "$withval" = "no"; then
        ...
        want_llvm="yes"
        ac_llvm_config_path=`which llvm-config-3.5`
    else
	    want_llvm="yes"
	...
	if test "x$want_llvm" = "xyes"; then
		...
		if test "$succeeded" != "yes" ; then
			AC_MSG_ERROR([[Unable to detect the llvm libraries. Make sure that llvm-config-3.5 is on your path or specified by --with-llvm. $ac_llvm_config_path]])
		else
